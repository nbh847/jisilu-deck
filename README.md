...
jisilu-deck 在页面原有操作按钮右侧增加一套独立的本地自选：橙色 `+` 用于加入，红色 `-` 用于移出。记录只保存在当前浏览器中，因此本地自选数量不受站内 10 项额度限制。可转债本地自选还可以用名称旁的灰色 `+` 标记为“待购”。
...
- 加入本地自选后，转债名称保持页面原本颜色。
- 本地自选转债的名称旁显示灰色 `+`；点击后变为橙色“待购”，再次点击直接清除待购状态。
- 在页面顶部增加“仅看本地自选／仅看待购”组合筛选，只显示当前站内筛选结果中的对应记录，两个筛选互斥。
...
- 页面原有按钮位于左侧，本地按钮位于右侧。
- 点击橙色 `+`：把当前转债加入本地自选，按钮变为红色 `-`，转债名称保持页面原本颜色。
- 点击红色 `-`：从本地自选移出当前转债，按钮和名称恢复原状态。
- 点击转债名称旁的灰色 `+`：标记为橙色“待购”；点击“待购”直接清除状态，本地自选仍保留。
- 点击“仅看本地自选”或“仅看待购”：只显示对应记录；两个按钮互斥，再次点击当前按钮恢复站内当前结果。
...
待购记录只保存 `bondCode` 和加入待购时间，并且必须从属于本地自选；移出本地自选会同时清除待购状态。待购不代表已成交，扩展不保存购买日期、数量、价格或持仓。
